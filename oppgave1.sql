...
-- Author: <your name>
-- NOTE: This script (re)creates the database from scratch and loads example data.
--       Do NOT run if the database already exists and contains data you want to keep.
-- ============================================
...
-- eksemplar: physical copies of books; composite PK (ISBN, EksNr)
...
-- låner: borrowers; auto-increment PK
CREATE TABLE `låner` (
...
-- utlån: loans; references both låner and eksemplar
CREATE TABLE `utlån` (
...
('8203191201','Victoria','Hamsun, Knut','Gyldendal',1895,164),
('8253029533','Jonas','Bjørneboe, Jens','Pax',1955,321),
...
SELECT COUNT(*) AS `antall_bøker` FROM `bok`;
SELECT `ISBN`, COUNT(*) AS `antall_eksemplar` FROM `eksemplar` GROUP BY `ISBN`;
SELECT * FROM `låner` ORDER BY `LNr`;
